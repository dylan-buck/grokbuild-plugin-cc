...
const DEFAULT_INLINE_DIFF_MAX_BYTES = 400 * 1024;
  ...
  return {
    ...
    content: includeDiff
      ? [
          ...
          formatSection(
            "Branch Diff",
            gitChecked(cwd, ["diff", "--no-ext-diff", "--submodule=diff", comparison.commitRange]).stdout
          )
        ].join("\n")
      : [
          formatSection("Commit Log", logOutput),
...
function buildCollectionGuidance(options = {}) {
  if (options.includeDiff !== false) {
    return "Use the repository context below as primary evidence. Do not use tools; review the embedded diff as given.";
  }

  return "The repository context below is a lightweight summary only. The full diff was too large to embed.";
}
...
export function collectReviewContext(cwd, target, options = {}) {
  ...
  if (target.mode === "working-tree") {
    ...
    diffBytes = measureCombinedGitOutputBytes(
      ...
      [
        ...
        ["diff", "--no-ext-diff", "--submodule=diff"]
      ],
      maxInlineDiffBytes
    );
    includeDiff =
    ...
    diffBytes = measureGitOutputBytes(
      repoRoot,
      ["diff", "--no-ext-diff", "--submodule=diff", comparison.commitRange],
      maxInlineDiffBytes
    );
  ...
  return {
    ...
    diffBytes,
    inputMode: includeDiff ? "inline-diff" : "summary-only",
