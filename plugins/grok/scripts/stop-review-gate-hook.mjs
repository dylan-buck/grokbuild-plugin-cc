...
function runStopReview(cwd, input = {}) {
  ...
  // Read-only stop-gate review (no --write).
...
function main() {
  ...
  // Avoid re-entering the gate when the last job was already a stop-gate run.
  const latestFinished = jobs.find((job) => job.status !== "queued" && job.status !== "running");
  if (latestFinished?.summary?.includes("Stop-gate") || latestFinished?.title?.includes("Stop Gate")) {
  ...
  // Skip pure status/setup turns with no code-change signal in the assistant message.
